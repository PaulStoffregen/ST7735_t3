...
ST7735_t3::ST7735_t3(uint8_t cs, uint8_t rs, uint8_t sid, uint8_t sclk, uint8_t rst, uint8_t miso)
{
	_cs   = cs;
	_rs   = rs;
	_sid  = sid;
...
void ST7735_t3::setCursor(int16_t x, int16_t y) {
	if (x < 0) x = 0;
...
void ST7735_t3::setTextSize(uint8_t s) {
  textsize = (s > 0) ? s : 1;
}
...
int16_t ST7735_t3::strPixelLen(char * str)
{
...
void Adafruit_GFX_Button::initButton(ST7735_t3 *gfx,
